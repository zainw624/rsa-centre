import { TRACKED_ROLES, permissionForRoles, isIgnoredRole } from '@/lib/permissions';
...
export function mapDiscordRoles(roleIds: string[], roleMap: Record<string, string>) {
  return roleIds
    ...
    .filter((name): name is string =>
      typeof name === 'string' && !isIgnoredRole(name) && TRACKED_ROLES.includes(name));
}

export function resolvePermission(roleNames: string[], botOwnerId?: string, userId?: string) {
  return permissionForRoles(roleNames, Boolean(botOwnerId && userId === botOwnerId));
}
